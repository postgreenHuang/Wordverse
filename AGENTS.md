...
- Wordverse is a multi-project application. A project is an independent spatial knowledge workspace for one context such as work, private learning, or language study; graphs, annotations, assets, trash, backups, conflicts, and revision history never cross project boundaries.
- On first launch, or when the last project is unavailable, show a quiet project launcher with recent projects, "open existing project", "create project", and the legacy `%USERPROFILE%\.Wordverse` option. This launcher is an operational workspace chooser, never a marketing screen.
- When the previous project still exists and validates successfully, launch directly into its last editing scene. The active project and recent-project registry are device-local application state.
- A project root contains `.wordverse-project.json`, `settings.json`, `词网`, `assets`, and a private `.wordverse` directory for `backups` and `conflicts`. The manifest has a stable project ID, display name, and project-format version. Never infer project identity from a mutable folder name.
- The existing `%USERPROFILE%\.Wordverse` workspace upgrades in place as the protected legacy/default project. Migration first validates current data, writes the project manifest atomically, and never deletes or relocates legacy data before a successful load and save.
- Native project creation and opening use an operating-system folder picker. Canonicalize and validate the selected root, reject files, project nesting, inaccessible roots, malformed manifests, and paths that escape the active project.
- Rust owns the active canonical project context. Frontend storage commands must not accept arbitrary asset or workspace paths; every load, save, backup, restore, conflict, import, export, and image operation resolves beneath the active project root.
- `StoragePort` is bound to an explicit project context rather than a process-wide fixed `.Wordverse` singleton. Switching projects flushes and waits for the current serialized save queue, stops autosave, stores the current session, clears undo/redo and transient interaction state, activates the new project, then restores its workspace and scene session. A failed flush blocks switching and keeps the current project open.
- Project data is portable and syncable; device view state is not. Theme, shortcuts, panel sizes, recent projects, last project, and per-project scene sessions belong to the Tauri application-config area or device-local browser storage, never project JSON.
- Namespace tabs, graph depth paths, selections, hierarchy expansion, camera snapshots, and other scene session keys by stable project ID. Image-resolution caches must also include project ID so equal relative asset names cannot leak across projects.
- The Project Browser continues to list only top-level graph files inside the active Wordverse project. Switching Wordverse projects is an app-level action in the brand/project menu, not another hierarchy level in the graph browser.
- Missing, moved, read-only, or externally modified projects produce recoverable states: locate again, remove from recents, open read-only where supported, or cancel. Never silently create a replacement project at a missing path.
## Visual direction
